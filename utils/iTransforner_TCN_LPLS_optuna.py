...
from models import iTransformer_TCN
from utils.tools import metrics_of_pv, EarlyStopping, same_seeds
from utils.loss import MLE_Gaussian
import warnings
...
def evaluate(data, model,criterion, batch_size=64, device=torch.device("cuda:0")):
    ...
    for x,y in tqdm(data):
        ...
        with torch.no_grad():
            ...
            all_preds.extend(y_pre[:,:,0].cpu().numpy())
...
if __name__ == "__main__":
    ...
    data_train, data_valid, data_test, timestamp_train, timestamp_valid, timestamp_test, scalar = split_data(df_all, 0.8, 0.1, time_length)
    ...
    def objective(trial):
        ...
        model=iTransformer_TCN(input_size=5,num_channels=[channels]*layer_T,kernel_size=kernel_size,
        ...
        model_name = "iTransformer_TCN_LPLS"
    ...
    study = optuna.create_study(direction='minimize', sampler=optuna.samplers.TPESampler(),load_if_exists=True,
                                storage=f'sqlite:///db.sqlite3', study_name=f'{site}_{dataset}_{predict_length}_lpls')
