...
class hierarchy_Mamba_model(nn.Module):
    def __init__(self, pred_length=6,d_model=128,d_conv=3,d_ff=128,expand=2):
        super(hierarchy_Mamba_model, self).__init__()

        self.base_model1 =Mamba_model(pred_len=int(pred_length)*7,enc_in=1,d_model=d_model,d_conv=d_conv,d_ff=d_ff,expand=expand,c_out=1)
        ...
        self.base_model2 = Mamba_model(pred_len=int(pred_length/3)*7,enc_in=1,d_model=d_model,d_conv=d_conv,d_ff=d_ff,expand=expand,c_out=1) # 池化大小为3的模型

        self.base_model3 = Mamba_model(pred_len=int(pred_length/6)*7,enc_in=1,d_model=d_model,d_conv=d_conv,d_ff=d_ff,expand=expand,c_out=1) # 池化大小为6的模型
